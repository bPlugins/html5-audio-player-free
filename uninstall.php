...
// -------------------------------------------------------------------------
$audioplayer_ids = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
...
// -------------------------------------------------------------------------
$radioplayer_ids = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
...
foreach ( $h5ap_taxonomies as $taxonomy ) {
	...
	if ( ! empty( $term_ids ) ) {
		foreach ( $term_ids as $term_id ) {
			$wpdb->delete( $wpdb->term_relationships, array( 'term_taxonomy_id' => (int) $term_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->delete( $wpdb->term_taxonomy, array( 'term_taxonomy_id' => (int) $term_id ) );      // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->delete( $wpdb->terms, array( 'term_id' => (int) $term_id ) );                       // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		}
	}
}
